...
func manifestsEqual(new, old []workapiv1.Manifest) bool {
	...
	for i := range new {
		if !equality.Semantic.DeepEqual(manifestUnstructured(new[i]), manifestUnstructured(old[i])) {
			return false
		}
	}
	return true
...
func manifestUnstructured(manifest workapiv1.Manifest) *unstructured.Unstructured {
	newManifest := &unstructured.Unstructured{}
	if err := newManifest.UnmarshalJSON(manifest.Raw); err != nil {
		klog.Errorf("failed to unmarshal new manifest. err:%v", err)
		return newManifest
	}

	unstructured.RemoveNestedField(newManifest.Object, "metadata", "creationTimestamp")
	unstructured.RemoveNestedField(newManifest.Object, "spec", "template", "metadata", "creationTimestamp")
...
func ManifestWorkEqual(new, old *workapiv1.ManifestWork) bool {
	...
	if !equality.Semantic.DeepEqual(new.Spec.Executor, old.Spec.Executor) {
		return false
	}
	if !equality.Semantic.DeepEqual(new.Annotations, old.Annotations) {
		return false
	}
	if !equality.Semantic.DeepEqual(new.Labels, old.Labels) {
		return false
	}
	if !equality.Semantic.DeepEqual(new.OwnerReferences, old.OwnerReferences) {
		return false
	}
	return true
}
